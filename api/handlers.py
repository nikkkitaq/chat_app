...
async def _delete_user(user_id: UUID, db: AsyncSession = Depends(get_db)) -> Optional[UUID]:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
...
async def _update_user(user_id: UUID, updated_user_params: dict, db: AsyncSession = Depends(get_db)) -> Optional[UUID]:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
...
async def _get_user_by_id(user_id: UUID, db: AsyncSession = Depends(get_db)) -> Optional[ShowUser]:
    async with db as session:
        async with session.begin():
            user_dal = UserDAL(session)
...
async def update_user_by_id(
    ...
    if user is None:
        raise HTTPException(status_code=404, detail=f"User with id {user_id} not found.")
    try:
        updated_user_id = await _update_user(user_id=user_id, updated_user_params=updated_user_params, db=db)
    except IntegrityError as err:
        logger.error(err)
        raise HTTPException(status_code=503, detail=f"Database error: {err}")
